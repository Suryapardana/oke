/**
   * Made By Fandyyy 🕴️
   * No Enc 100% Chat wa.me/6285609233482
*/
...
cfonts.say('Whatsapp Bot By Angga',{
font: 'console',
gradient: ['red','magenta'],
align: 'center'
})
...
//anticall
ichi.ev.on('call', async (ichii) => {
let botNumber = await ichi.decodeJid (ichi.user.id)
let ichigo = db.data.settings[botNumber].anticall
if (!ichigo) return
console.log(ichii)
for (let chi of ichii) {
if (chi.isGroup === false) {
if (chi.status === 'offer') {
let pa7rick = await ichi.sendMessage(chi.from, { text: `*${ichi.user.name}* tidak bisa menerima panggilan ${chi.isVideo ? `video` : `suara`}. Maaf kamu akan diblockir. Jika tidak sengaja silahkan hubungi Owner untuk dibuka !`, mentions: [chi] })
ichi.sendContact(chi.from, global.ownerNumber, pa7rick)
await sleep(8000)
await ichi.updateBlockStatus(chi.from, "block")
}
}
}
})
...
if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	...
	lolcatjs.fromString(`Berhasil Tersambung Ke ` + JSON.stringify(ichi.user, null, 2))
	global.creator = ['6287891518799']
	...
	let butown = [{urlButton: {displayText: 'Creator 👦️',url: 'https://wa.me/6285609233482?text=Assalamualaikum'}}, {urlButton: {displayText: 'Subscribe 📱️',url: 'https://www.youtube.com/c/NZRLAFNDI'}}, {urlButton: {displayText: 'Grup Creator 📺️',url: 'https://chat.whatsapp.com/JtBShM3zwb06yWVSr7EGu7'}}]
	let butcrea = [{urlButton: {displayText: 'Owner 👦️',url: `https://wa.me/${global.Rowner}?text=p`}}]
	...
	ichi.sendMessage(global.Rowner+'@s.whatsapp.net', { image: imgown, caption: txtown, footer: global.ownerName })
    ichi.sendMessage('6285609233482@s.whatsapp.net', { image: imgcrea, caption: txtcrea, footer: global.ownerName })
    ichi.sendContact(global.Rowner+'@s.whatsapp.net', global.creator)
	}
} catch (err) {
